...
export const useBuilderStore = create<BuilderState & BuilderActions>()(
  persist(
    (set, get) => ({
      ...DEFAULT_STATE,
      setStep: (step) =>
        set({ step: Math.max(1, Math.min(TOTAL_STEPS, Math.floor(step))) }),
      next: () =>
        set({ step: Math.min(TOTAL_STEPS, get().step + 1) }),
      prev: () =>
    ...
    }),
    {
      ...
      partialize: (state) => {
        ...
        const {
          setStep: _s, next: _n, prev: _p, reset: _r, update: _u,
          addField: _a, removeField: _rm, updateField: _uf,
          ...persisted
        } = state as BuilderState & BuilderActions
        void _s; void _n; void _p; void _r; void _u; void _a; void _rm; void _uf
        return persisted
      },

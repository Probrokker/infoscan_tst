...
export function LoadingScreen() {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 bg-zinc-50">
      ...
      <div className="font-mono text-xs text-red-600">v 2.4.1</div>
    </div>
  )
}
    ...
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="flex flex-1 gap-3 p-3">
        ...
        <div className="flex w-32 flex-col items-center justify-center gap-2 rounded bg-zinc-100 p-2">
          ...
          {barcode && <div className="text-center font-mono text-[10px] break-all">{barcode}</div>}
        </div>
      </div>
  ...
  return (
    <div className="flex items-baseline justify-between rounded bg-zinc-50 px-3 py-2">
      ...
      <span className="font-mono text-xl font-bold tabular-nums">
        {value == null ? '—' : digits > 0 ? value.toFixed(digits) : formatNumber(Math.round(value))}
      </span>
    </div>
  )
}
...
function CalibrationLayout({
  ...
  return (
    <div className="flex h-full flex-col bg-zinc-50 p-4">
      <div className="mb-3 flex items-center justify-between">
        <button className="text-sm text-zinc-600" onClick={back}>
          ← Настройки
        </button>
  ...
  return (
    <div className="flex h-full flex-col bg-zinc-50 p-4">
      ...
      <div className="mx-auto grid w-64 grid-cols-3 gap-2">
        ...
        <button className="h-12 rounded bg-zinc-200 text-sm" onClick={() => setItemTypeCount(0)}>
          Сброс
        </button>
        <button
          className="h-12 rounded bg-white text-xl font-bold shadow"

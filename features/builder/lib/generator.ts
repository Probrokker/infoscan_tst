...
import type { BuilderState, Transport, AuthKind, BodyFormat, FieldMapping, TargetModel } from '../model/schema'
...
function genCapture(state: BuilderState, key: Transport): string {
  ...
  lines.push(`${INDENT}${INDENT}label: { text: 'BARCODE: ' + BARCODE, pointSize: 20, color: '#FFCE00' },`)
...
export function generateScript(state: BuilderState): string {
  ...
  const guard =
    `var num = ${lengthVariableFor(state.targetModel)};${NL}if (num == null) { num = 0 }${NL}`

  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select value={targetModel} onValueChange={(v) => update({ targetModel: v as typeof targetModel })}>
          <SelectTrigger>
        ...
        <p className="mt-2 text-xs text-[var(--muted-foreground)]">
          Для Camera используется <code className="font-mono">SENSOR.depthFormat</code>, для 3D 60 / 3D 90 —{' '}
          <code className="font-mono">SENSOR.lengthFormat</code>. Конструктор подставит правильное имя автоматически.
        </p>
      </div>
...
const TARGET_SYSTEMS: Array<{ id: TargetSystem; name: string; note: string }> = [
  { id: '1c', name: '1С (УТ / ERP / УПП)', note: 'Обычно HTTP-сервис на стороне 1С или Apache+PHP-прослойка.' },
  { id: 'solvo', name: 'Solvo WMS', note: 'Поддерживает REST API и SOAP. Уточняйте у Solvo формат для версии.' },
  ...
  { id: 'magnit', name: 'Магнит (внутренний WMS)', note: 'SOAP с заголовками X-Tander-* (есть пример).' },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-[var(--muted-foreground)]">
          Куда отправляем результаты измерений.
        </p>
      </div>

      <Select value={targetSystem} onValueChange={(v) => update({ targetSystem: v as TargetSystem })}>
        <SelectTrigger>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-[var(--muted-foreground)]">
          Что именно отправляем и в каком виде.
        </p>
      </div>
          ...
          <ul className="space-y-2">
            {fields.map((f, i) => (
              <li key={i} className="flex gap-2">
                ...
                <Select
                  value={f.source}
                  onValueChange={(v) => updateField(i, { ...f, source: v })}
                >
  ...
  return (
    <div className="space-y-6">
        ...
        {customHeaders.map((h, i) => (
          <div key={i} className="mb-2 flex gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                update({ customHeaders: customHeaders.filter((_, idx) => idx !== i) })
              }
      ...
      <Callout kind="warning" title="Секреты — только в .env">
        Этот конструктор хранит введённые значения только в вашем браузере (localStorage). Не вставляйте сюда боевые
        токены — используйте подмену через переменные окружения и шаблон{' '}
